...
internal object AllureMetadataSupportLinks {
   ...
   internal inline val KClass<out Spec>?.linkAnnotations: Collection<Link>
      get() = this?.let { getLinks(it.java) }.orEmpty()

   internal inline val KClass<out Spec>?.issues: Collection<Link>
      get() = this?.findAnnotation<Issues>()?.value?.map { createLink(it) } ?: emptySet()

   internal inline val KClass<out Spec>?.links: Collection<Link>
      get() = this?.findAnnotation<Links>()?.value?.map { createLink(it) } ?: emptySet()

   internal inline val KClass<out Spec>?.tmsLinks: Collection<Link>
      get() = this?.findAnnotation<TmsLinks>()?.value?.let { getLinks(it.toSet()) } ?: emptySet()

   internal inline val KClass<out Spec>?.taskLinks: Collection<Link>
      get() = this?.findAnnotation<KTasks>()?.value?.let { getLinks(it.toSet()) } ?: emptySet()
